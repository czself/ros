...
ROUTE=[
 ...
 (-3.97,-2.62),(4.08,-2.80),(4.08,-3.60),(4.08,-4.25),
 (3.53,-4.25),(3.03,-4.25),(2.53,-4.25),(2.03,-4.25),(1.53,-4.25),(1.03,-4.25),(0.53,-4.25),(0.03,-4.25),(-0.47,-4.25),(-0.97,-4.25),(-1.47,-4.25),(-1.97,-4.25),(-2.47,-4.25),(-2.97,-4.25),(-3.47,-4.25),(-3.90,-4.30)]
MAX_DURATION=1500.0
 ...
 def run(self):
  ...
  try:
   while not rospy.is_shutdown() and self.i<len(ROUTE):
    ...
    # A block is only meaningful while the robot is actually driving forward at
    # a corridor-aligned heading (forward velocity starts at |e| <= 0.20).  A
    # pure in-place rotation carries no linear velocity, so requiring |e| small
    # here prevents a wide initial yaw sweep past a nearby wall from falsely
    # aborting the survey; the aligned-heading protection is unchanged.
    if front<.50 and abs(e)<.35: self.stop_reason='blocked';raise RuntimeError('obstacle blocked route')
